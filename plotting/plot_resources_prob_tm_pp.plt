...
datafile = "output/carlos_indep_pp/resources_prob_tm_NP_pp.txt"
...
datafile = "output/carlos_indep_pp/resources_prob_tm_PW_pp.txt"
...
# (cycle 0 is excluded: it is off-scale on the log axis).
fitmin = 7
fitmax = 25
f(x) = a*x**gamma
array pw_a[nb]
...
print sprintf("\n-- %s --", datafile)
do for [i=1:nb] {
    # Blocks that are flat or have < 3 points in range are not fittable
    # (γ is undetermined and the fit hits a singular matrix); mark them NaN
    # so both the curve and its key entry drop out below. Also require >= 3
    # non-zero-error points, since those are all the weighted fit can use.
...
datafile = "output/carlos_indep_pp/resources_prob_tm_PC_pp.txt"
