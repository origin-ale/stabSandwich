...
set title "Random rotation magnetization dynamics, N = 15" font ",24"
...
plot "output/CircuitDynamics.txt" index 0 with linespoints title "CAMPS-PP (χ = 64, Nmax = 200)" lc rgb "blue", \
     "output/CircuitDynamics.txt" index 1 with linespoints title "CAMPS (χ = 64)" lc rgb "red", \
     "output/CircuitDynamics.txt" index 2 with linespoints title "Pauli propagation (Nmax = 200)" lc rgb "dark-green"